...
export class ExamSessionsService {
  async create(userUuid: string, category: Category) {
    ...
    for (const bucket of PLAN) {
      const assignments = await db.orm.public.CategoryAssignment
      .where({ category })
      .select('questionUuid')
      .all()
      ...
      const matching = candidates.filter((q) => candidateUuids.includes(q.uuid))
    ...
    const session = await db.orm.public.ExamSession.create({ userUuid, category });
  ...
  async getCurrentQuestion(examSessionUuid: string) {
    ...
    const allQuestions = await db.orm.public.ExamSessionQuestion
      .where({ 
        examSessionUuid 
      })
      .all();

    const answered = await db.orm.public.UserAnswer
      .where({ 
        examSessionUuid 
      })
      .all();
    ...
    if (!next.presentedAt) {
      await db.orm.public.ExamSessionQuestion
        .where({ 
          uuid: next.uuid 
        })
        .update({ 
    ...
    const question = await db.orm.public.Question.where({ uuid: next.questionUuid }).first();
  ...
  async addAnswer(examSessionUuid: string, questionUuid: string, givenAnswer: Answer) {
    await this.assertSessionActive(examSessionUuid);

    const link = await db.orm.public.ExamSessionQuestion
      .where({ 
        examSessionUuid, questionUuid 
      })
      .first();
  ...
  private async maybeFinalizeSession(examSessionUuid: string) {
    ...
    const totalQuestions = await db.orm.public.ExamSessionQuestion
      .where({ 
        examSessionUuid 
      })
      .all();

    const givenAnswers = await db.orm.public.UserAnswer
      .where({ 
        examSessionUuid 
      })
      .all();
  ...
  private async handleExpiredQuestions(examSessionUuid: string) {
    const allLinks = await db.orm.public.ExamSessionQuestion
      .where({ 
        examSessionUuid 
      })
      .all();
    ...
    const question = await db.orm.public.Question.where({ uuid: pending.questionUuid }).first();
    const limit = TIME_LIMITS_SECONDS[question!.level];
