...
const DOWNLOAD_IMPORT_FILES = process.env.DOWNLOAD_IMPORT_FILES?.toLowerCase() !== 'false' && process.env.DOWNLOAD_IMPORT_FILES !== '0';
...
async function main(): Promise<void> {
  ...
  if (DOWNLOAD_IMPORT_FILES) {
    ...
    console.log('DOWNLOAD_IMPORT_FILES=false. Skipping download and extraction, using existing files in WORK_DIR.');
    
    if (!fs.existsSync(catalogPath)) {
      throw new Error(`Catalog file missing at ${catalogPath}`);
    }
    if (!fs.existsSync(mediaExtractDir)) {
      throw new Error(`Extracted media directory missing at ${mediaExtractDir}`);
    }
  }
